...
If you want to train with AWS located at multiple regions, then you can use the following command:
...
def run_command(instance, client, cmd, environment=None, inputs=None):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=2) as printer:
        # if instance == "i-0b0089e3369d9dee5":
...
def main():
    ...
    if len(regions) > 1:
        ...
        assert len(instance_ids) == len(
            ssh_key_files
        ), "{} instance ids are provided, but {} ssh_key_file found.".format(
        ...
        assert len(ssh_key_files) == 1, "1 region is detected, but {} ssh_key_file found.".format(
            len(ssh_key_files)
        )
    ...
    environment = {
        ...
        "RENDEZVOUS": "env://",
        "MASTER_ADDR": master_instance.public_ip_address,
        "MASTER_PORT": "23456",
    }
