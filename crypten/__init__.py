...
def init(party_name=None, device=None, use_csprng=False, sync_key=False):
    """
    ...
    if comm.get().get_rank() < comm.get().get_world_size():
        _setup_przs(device=device, use_csprng=use_csprng, sync_key=sync_key)
...
def _setup_przs(device=None, use_csprng=False, sync_key=False ):
    """
    ...
    if use_csprng:
        if sync_key:
            ...
            g0_key = csprng.aes128_key_tensor(urandom_gen)
            g1_key = csprng.aes128_key_tensor(urandom_gen)

            comm.get().g0 = csprng.create_const_generator(g0_key)
            comm.get().g1 = csprng.create_const_generator(g1_key)

            world_size = comm.get().get_world_size()

            next_key = csprng.aes128_key_tensor(urandom_gen)
            prev_key = torch.empty_like(next_key)
            ...
            req0 = comm.get().isend(tensor=next_key, dst=next_rank)
            req1 = comm.get().irecv(tensor=prev_key, src=prev_rank)

            req0.wait()
            req1.wait()

            comm.get().g0 = csprng.create_const_generator(next_key)
            comm.get().g1 = csprng.create_const_generator(prev_key)

            comm.get().g0_cuda = csprng.create_const_generator(next_key)
            comm.get().g1_cuda = csprng.create_const_generator(prev_key)
        else:
            comm.get().g0 = csprng.create_random_device_generator('/dev/urandom')
            comm.get().g1 = csprng.create_random_device_generator('/dev/urandom')

            if torch.cuda.is_available():
                comm.get().g0_cuda = comm.get().g0
                comm.get().g1_cuda = comm.get().g1
